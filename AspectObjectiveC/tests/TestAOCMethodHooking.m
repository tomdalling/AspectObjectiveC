...
-(void) doTestInstallAndUninstallForSelector:(SEL)selector;
{
    ...
    NSInvocation* selectorInv = [NSInvocation invocationWithMethodSignature:[cls instanceMethodSignatureForSelector:selector]];
    [selectorInv setSelector:selector];
    [selectorInv setTarget:self];
...
/***************************************
 * This was generated by generate_method_hooking_tests.rb
 * Don't edit this section because it will be overwritten on next build
 * Edit generate_method_hooking_tests.rb instead
 ***************************************/
...
-(void) testReturnNSRect1;{
    STAssertFalse(g_hookDidRun, @"Hook shouldn't have run yet");
    m_rect = NSZeroRect;
    STAssertEquals([self returnNSRect], m_rect, @"Return value is mangled");
    STAssertTrue(g_hookDidRun, @"Hook should have run by now");
}
-(void) testReturnNSRect2;{
    STAssertFalse(g_hookDidRun, @"Hook shouldn't have run yet");
    m_rect = NSMakeRect(1,2,3,4);
    STAssertEquals([self returnNSRect], m_rect, @"Return value is mangled");
    STAssertTrue(g_hookDidRun, @"Hook should have run by now");
}
-(NSRect) returnNSRectForInstall; {
    NSLog(@"inside %@", NSStringFromSelector(_cmd));
    return m_rect;
}
-(void) testInstallAndUninstallForReturnTypeNSRect; {
    SEL selector = @selector(returnNSRectForInstall);
    [self doTestInstallAndUninstallForSelector:selector];
}


// NSPoint /////////////////////////
-(NSPoint) returnNSPoint; {
    NSLog(@"inside %@", NSStringFromSelector(_cmd));
    return m_point;
}
-(void) testReturnNSPoint1;{
    STAssertFalse(g_hookDidRun, @"Hook shouldn't have run yet");
    m_point = NSZeroPoint;
    STAssertEquals([self returnNSPoint], m_point, @"Return value is mangled");
    STAssertTrue(g_hookDidRun, @"Hook should have run by now");
}
-(void) testReturnNSPoint2;{
    STAssertFalse(g_hookDidRun, @"Hook shouldn't have run yet");
    m_point = NSMakePoint(7,9);
    STAssertEquals([self returnNSPoint], m_point, @"Return value is mangled");
    STAssertTrue(g_hookDidRun, @"Hook should have run by now");
}
-(NSPoint) returnNSPointForInstall; {
    NSLog(@"inside %@", NSStringFromSelector(_cmd));
    return m_point;
}
-(void) testInstallAndUninstallForReturnTypeNSPoint; {
    SEL selector = @selector(returnNSPointForInstall);
    [self doTestInstallAndUninstallForSelector:selector];
}
...
-(void) testReturnNSSize1;{
    STAssertFalse(g_hookDidRun, @"Hook shouldn't have run yet");
    m_size = NSZeroSize;
    STAssertEquals([self returnNSSize], m_size, @"Return value is mangled");
    STAssertTrue(g_hookDidRun, @"Hook should have run by now");
}
-(void) testReturnNSSize2;{
    STAssertFalse(g_hookDidRun, @"Hook shouldn't have run yet");
    m_size = NSMakeSize(2,5);
    STAssertEquals([self returnNSSize], m_size, @"Return value is mangled");
    STAssertTrue(g_hookDidRun, @"Hook should have run by now");
}
-(NSSize) returnNSSizeForInstall; {
    NSLog(@"inside %@", NSStringFromSelector(_cmd));
    return m_size;
}
-(void) testInstallAndUninstallForReturnTypeNSSize; {
    SEL selector = @selector(returnNSSizeForInstall);
    [self doTestInstallAndUninstallForSelector:selector];
}
...
// NSRect /////////////////////////
-(void) assertArgEqualOfTypeNSRect:(NSRect)arg;{
    STAssertEquals(arg, m_rect, @"Argument is mangled");
}
-(void) testArgEqualForNSRect1; {
    STAssertFalse(g_hookDidRun, @"Hook shouldn't have run yet");
    m_rect = NSZeroRect;
    [self assertArgEqualOfTypeNSRect:m_rect];
    STAssertTrue(g_hookDidRun, @"Hook should have run by now");
}
-(void) testArgEqualForNSRect2; {
    STAssertFalse(g_hookDidRun, @"Hook shouldn't have run yet");
    m_rect = NSMakeRect(1,2,3,4);
    [self assertArgEqualOfTypeNSRect:m_rect];
    STAssertTrue(g_hookDidRun, @"Hook should have run by now");
}
-(void) installMethodForArgTypeNSRect:(NSRect)arg;{
    NSLog(@"Inside %@", NSStringFromSelector(_cmd));
}
-(void) testInstallAndUninstallForArgTypeNSRect; {
    SEL selector = @selector(installMethodForArgTypeNSRect:);
    [self doTestInstallAndUninstallForSelector:selector];
}


// NSPoint /////////////////////////
-(void) assertArgEqualOfTypeNSPoint:(NSPoint)arg;{
    STAssertEquals(arg, m_point, @"Argument is mangled");
}
-(void) testArgEqualForNSPoint1; {
    STAssertFalse(g_hookDidRun, @"Hook shouldn't have run yet");
    m_point = NSZeroPoint;
    [self assertArgEqualOfTypeNSPoint:m_point];
    STAssertTrue(g_hookDidRun, @"Hook should have run by now");
}
-(void) testArgEqualForNSPoint2; {
    STAssertFalse(g_hookDidRun, @"Hook shouldn't have run yet");
    m_point = NSMakePoint(7,9);
    [self assertArgEqualOfTypeNSPoint:m_point];
    STAssertTrue(g_hookDidRun, @"Hook should have run by now");
}
-(void) installMethodForArgTypeNSPoint:(NSPoint)arg;{
    NSLog(@"Inside %@", NSStringFromSelector(_cmd));
}
-(void) testInstallAndUninstallForArgTypeNSPoint; {
    SEL selector = @selector(installMethodForArgTypeNSPoint:);
    [self doTestInstallAndUninstallForSelector:selector];
}


// NSSize /////////////////////////
-(void) assertArgEqualOfTypeNSSize:(NSSize)arg;{
    STAssertEquals(arg, m_size, @"Argument is mangled");
}
-(void) testArgEqualForNSSize1; {
    STAssertFalse(g_hookDidRun, @"Hook shouldn't have run yet");
    m_size = NSZeroSize;
    [self assertArgEqualOfTypeNSSize:m_size];
    STAssertTrue(g_hookDidRun, @"Hook should have run by now");
}
-(void) testArgEqualForNSSize2; {
    STAssertFalse(g_hookDidRun, @"Hook shouldn't have run yet");
    m_size = NSMakeSize(2,5);
    [self assertArgEqualOfTypeNSSize:m_size];
    STAssertTrue(g_hookDidRun, @"Hook should have run by now");
}
-(void) installMethodForArgTypeNSSize:(NSSize)arg;{
    NSLog(@"Inside %@", NSStringFromSelector(_cmd));
}
-(void) testInstallAndUninstallForArgTypeNSSize; {
    SEL selector = @selector(installMethodForArgTypeNSSize:);
    [self doTestInstallAndUninstallForSelector:selector];
}
...
+(void) initializeNecessaryHooks; {
    ...
    AOCInstallHook(MockHookForTesting, NULL, [self class], @selector(returnNSRect), nil);
    AOCInstallHook(MockHookForTesting, NULL, [self class], @selector(returnNSRectKVCAccessorHooked), nil);
    AOCInstallHook(MockHookForTesting, NULL, [self class], @selector(returnNSPoint), nil);
    AOCInstallHook(MockHookForTesting, NULL, [self class], @selector(returnNSPointKVCAccessorHooked), nil);
    AOCInstallHook(MockHookForTesting, NULL, [self class], @selector(returnNSSize), nil);
    AOCInstallHook(MockHookForTesting, NULL, [self class], @selector(returnNSSizeKVCAccessorHooked), nil);
    //arg methods
    ...
    AOCInstallHook(MockHookForTesting, NULL, [self class], @selector(assertArgEqualOfTypeNSRect:), nil);
    AOCInstallHook(MockHookForTesting, NULL, [self class], @selector(assertArgEqualOfTypeNSPoint:), nil);
    AOCInstallHook(MockHookForTesting, NULL, [self class], @selector(assertArgEqualOfTypeNSSize:), nil);
}
